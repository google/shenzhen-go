...
<body>
	...
	<div class="formfield"><label for="Src">Source node</label><select name="Src">{{range .Graph.Nodes}}<option value="{{.Name}}" {{if $.Channel}}{{if eq .Name $.Channel.Src}}selected{{end}}{{end}}>{{.Name}}</option>{{end}}</select></div>
	<div class="formfield"><label for="Dst">Destination node</label><select name="Dst">{{range .Graph.Nodes}}<option value="{{.Name}}" {{if $.Channel}}{{if eq .Name $.Channel.Dst}}selected{{end}}{{end}}>{{.Name}}</option>{{end}}</select></div>
